#!/usr/bin/env dart
// Script to generate all 348 components from Figma
// Usage: dart scripts/generate_all_components.dart [--figma-token TOKEN]
...
void main(List<String> args) async {
  print('Sharpsell Component Generator');
  print('=============================');
  print('Total components to process: ${allNodeIds.length}');
  print('');
  
  // Check for Figma token
  final figmaToken = Platform.environment['FIGMA_TOKEN'];
  if (figmaToken == null) {
    print('⚠️  FIGMA_TOKEN not set. Components will be generated with placeholders.');
    print('   Set FIGMA_TOKEN environment variable to extract actual designs.');
    print('');
  }
  
  print('Starting component generation...');
  print('This will create components in lib/components/');
  print('and stories in lib/stories/');
  print('');
  ...
  for (int i = 0; i < allNodeIds.length; i += batchSize) {
    final batch = allNodeIds.sublist(
      i,
      i + batchSize > allNodeIds.length ? allNodeIds.length : i + batchSize,
    );
    
    print('Processing batch ${(i ~/ batchSize) + 1} (${batch.length} components)...');
    
    for (final nodeId in batch) {
      processed++;
      print('  [$processed/${allNodeIds.length}] Processing $nodeId...');
      
      // Generate component
      // This would call the component generator
  ...
  print('   Processed: $processed components');
  print('');
  print('Next steps:');
  print('1. Review generated components');
  print('2. Implement actual designs from Figma');
  print('3. Add proper props and documentation');
  print('4. Test in Storybook');
}
